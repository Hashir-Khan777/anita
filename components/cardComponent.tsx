...
const CardComponent = ({ cards }: any) => {
    ...
    const MySwiperComponent = dynamic(() => import("./SwiperComponent"), { ssr: false });

  return (
    <>
      <div className="hidden md:grid md:grid-cols-3 grid-cols-1 gap-5 px-4">
        {cards.map((card: any) => (
          <div
            key={card.id}
